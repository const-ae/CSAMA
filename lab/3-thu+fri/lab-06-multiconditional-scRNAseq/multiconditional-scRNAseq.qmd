...
In this tutorial, we will use `ggplot2` to visualize the data. This is often more verbose than calling an existing function (e.g., `scater::plotReducedDim(sce, "UMAP", color_by = "stim")`) but has the advantage that the plots are easier to customize.
...
There are many methods for single-cell data integration, and @luecken2022 benchmarked several approaches. Here, I will show how to do the integration manually or with `harmony`, before we will compare both methods to `LEMUR`.
