---
title: "Introduction to multi-condition data analysis with LEMUR"
...
![Figure 1 from *Analysis of multi-condition single-cell data with latent embedding multivariate regression* by @ahlmann-eltze2024. The goal is to identify which cells change expression in response to an experimental condition for each gene.](images/lemur_figure.png)
...
The call to `set.seed` to set the initial conditions ("seed") of R's random number generator, is optional. We make it here to ensure that the results are exactly the same for everyone. Some of the methods use stochastic sampling, but the expectation is that even with different random seeds, the results will be essentially, for all practical intents and purposes, the same.
...
In this tutorial, we use `ggplot2` to visualize the data. This is often more verbose than calling an existing function (e.g., `scater::plotReducedDim(sce, "UMAP", color_by = "stim")`) but has the advantage that the plots are easier to customize.
...
# Here we use `geom_text_repel` to avoid overlapping annotations
as_tibble(colData(sce)) |>
  mutate(umap = reducedDim(sce, "UMAP")) |>
...
@fig-kang-umap shows that the cell embedding locations separate by the treatment status. For many downstream analyses, it would be good to know how the cells from the stimulated condition are related to the cells from the control condition. For example for cell type assignment, we might want to annotate both conditions together, irrespective of treatment. This aspiration is called **integration**.

The goal is a low-dimensional embedding of the cells where the treatment status does not visibly affect the embedding and all residual variance comes from different cell states. @fig-integrated_picture shows an example.

![UMAP of a successfully integrated dataset.](images/integrated_data_picture.png){#fig-integrated_picture}

There are many approaches for single-cell data integration (the question is somewhat ill-defined and can be formalized into a mathematical algorithm in many different ways), and @luecken2022 benchmarked several approaches. Here, we show how to do the integration manually or with `harmony`, before we will compare both to `LEMUR`.
...
In transcriptomic data analysis, each cell is characterized by its gene expression profile. In our case, we decided to consider the 500 most variable genes. Accordingly, each cell is a point in a 500-dimensional _gene expression space_. Principal component analysis (PCA) finds a $P$-dimensional space ($P\le500$) such that overall, distances between each cell in the full space and the reduced space are minimized. 

To make these concepts more tangible, consider the cartoon in Figure @fig-ctrl-proj. The orange blob represents all cells from the control condition in a 3-dimensional gene expression space. The grey rectangle $R$ is a lower-dimensional subspace covering the shape of the orange blob. The shape of the blue blob (i.e., the treated cells) resembles the orange blob but is slightly offset. To integrate both conditions, we can project each point from the blue blob onto the subspace covering the orange blob.
...
LEMUR is a tool to analyze multi-condition single-cell data. The typical analysis workflow with LEMUR goes through four steps (@fig-lemur-workflow):
...
The functions in the `lemur` package mirror these steps:

```{r}
#| label: lemur-workflow
#| eval: false
#| message: false
# The following illustrates LEMUR's workflow
library(lemur)
# Step 1
...
In the next sections we will discuss all of these steps in more detail.
...
LEMUR can handle arbitrary design matrices as input which means that it can also handle much more complicated experiments than the treatment / control shown here. You can use LEMUR to adjust for known confounders in an experiment (such as experimental batch, sex, age). LEMUR can also be used to analyze time course experiments and supports spline regression to find non-linear expression trends. For more details see @ahlmann-eltze2024. If you have any questions, feel free to reach out to the authors (Constantin Ahlmann-Eltze and Wolfgang Huber) or post to the [Bioconductor forum](https://support.bioconductor.org/) using the `lemur` tag.
