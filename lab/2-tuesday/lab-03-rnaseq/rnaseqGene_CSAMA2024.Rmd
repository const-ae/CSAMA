...
Recall that we added the result columns from *DESeq2* to the 
`rowData()` of the *DESeqDataSet* object. Here, we instruct `r Biocpkg("iSEEu")`
(a companion package to *iSEE*, among other things providing 
panels for volcano plots and MA plots) that any column including 
`pvalue` in the name contains p-values, and similarly for log-fold changes and 
average abundances. These columns will be available for use in volcano plots 
and MA plots within the *iSEE* application. 

_Note_: A newer addition to the *iSEE* family - `r Biocpkg("iSEEde")` - 
provides another, perhaps more streamlined way of including differential 
expression results into *SummarizedExperiment* objects for use with *iSEE*. 
At the moment, both *iSEEu* and *iSEEde* define panels for MA plots and 
volcano plots - take care when loading the packages to make sure that you are 
